...
- Expand `$env.` directives during config loading so secret values can be injected from environment variables without manual parsing at each usage site. ([#132](https://github.com/nite-coder/bifrost/pull/132) by [0x5487](https://github.com/0x5487))
...
- Translated the Hot Reload and Chunked Transfer architecture specification documentation into English ([#118](https://github.com/nite-coder/bifrost/pull/118) by [0x5487](https://github.com/0x5487))
- Refactor high-risk `cast.B2S` and `cast.S2B` usage to use stable conversions in non-hot-path code ([#123](https://github.com/nite-coder/bifrost/pull/123) by [0x5487](https://github.com/0x5487))
- rename `internal.runtime` package to `infra` ([#125](https://github.com/nite-coder/bifrost/pull/125) by [0x5487](https://github.com/0x5487))
...
- Fully implement OTLP push mode support using OpenTelemetry Metrics SDK. Configuration is aligned with Tracing, featuring automatic protocol inference.  Maintains backward compatibility with existing Prometheus pull mode and plugin metrics via a Prometheus bridge ([#100](https://github.com/nite-coder/bifrost/pull/100) by [0x5487](https://github.com/0x5487))
